...
3. **后端 API 零改动**：仅静态文件服务方式改变（`GET /` 来源变为 `dist/` + 新增 `/assets` 挂载）；`/api/chat`、`/api/transcribe`、`/api/reset`、`/healthz` 全部不变。
...
| 构建产物 | **不提交 git**（`frontend/dist/`、`frontend/node_modules/` 进 `.gitignore`），README 增加 npm 前置步骤 |
...
- **SSE 流式解码**：`TextDecoder.decode(part, {stream:true})` + 尾部缓冲（分块可能切在 UTF-8 多字节字符中间）；`JSON.parse` 返回 `any`，需为 `ChatEvent` 编写 discriminated-union type guard（`isChatEvent`），满足禁 `any` 约束。
- **不使用 `public/` 目录**放资源：`public/` 产物直接拷到 `dist/` 根（`/favicon.ico` 等），不在 `/assets` 挂载覆盖范围内，会 404。

### 3.2 后端适配（`frontend/web.py`）

- **删除 `frontend/web.html`**（被 React 应用替换，不留死代码）；`pyproject.toml` 的 `[tool.setuptools.package-data] frontend = ["web.html"]` 同步移除（避免 setuptools 对不存在文件的告警）。
- `WEB_HTML` 常量改为指向 `PROJECT_ROOT / "frontend" / "dist" / "index.html"`。
- `create_app` 增加 `index_html: Path | None = None` 参数（沿用现有依赖注入风格）：`None` 时默认 `frontend/dist/index.html`；测试注入存在/缺失路径，使首页两种行为成为**确定性的独立用例**，不随环境漂移。
...
- **部署说明**：dist 产物只存在于源码树，`pip install -e .`（editable）场景下正常；非 editable 安装（wheel/sdist）不含 dist，`GET /` 恒 503 提示页——规格只保证源码树部署。
- 其余路由与逻辑不变。

### 3.3 测试更新（`tests/test_web.py`）

仓库惯例是 `pip install -e . && pytest` 即可全绿（无 Node 环境）。首页断言改为**注入式的两个独立用例**（通过 `create_app(index_html=...)`，不依赖真实文件系统）：

- 注入存在路径的 index.html → `GET /` 为 200，HTML 含标题「AI 花音」与挂载根节点（如 `<div id="root">`）；
- 注入缺失路径 → `GET /` 为 503，HTML 含提示文案「npm run build」。
...
**抽屉交互规范**（≤768px 时侧边栏折叠）：顶栏头像按钮唤出 → 半透明遮罩层；点击遮罩、按 ESC、或再次点击按钮关闭；打开时锁定 `body` 滚动；关闭后焦点返回触发按钮；遮罩与抽屉标注 `aria-hidden` / 按钮标注 `aria-expanded`。

### 4.3 聊天主区

- 顶栏：左「AI 花音」品牌字，右 `StatusBadge`（状态徽标：在线 / 生成中 / 录音中 / 识别中 / 请配置 DeepSeek API key / TTS 服务未启动 / 在线（语音不可用） / 连接异常）。
- 消息列表：居中 `max-width 760px` 消息列；AI 气泡暖紫灰渐变靠左、用户气泡深青渐变靠右，气泡带微妙渐变、边框与阴影。**新消息自动滚动到底部**（延续现状 `web.html:37` 行为；用户主动上翻浏览历史时不强制拉回）。
...
- **`useAudioQueue`**：单例队列，`enqueue(url)` → 顺序播放（`ended` 触发下一个）；`HTMLAudioElement` + `AudioContext.createMediaElementSource` → `analyser.getByteFrequencyData` → rAF 输出 24 条波形高度；暴露 `currentUrl`、`isPlaying`、`toggle(url)`。
  - **连接链必须完整**：`source.connect(analyser)` 后必须 `analyser.connect(ctx.destination)`——`createMediaElementSource` 会把元素从直接输出改道 AudioContext，不连 destination 则完全无声。**每个 audio 元素只创建一次 source**（重复调用抛 `InvalidStateError`）。
  - **AudioContext 手势解锁**：浏览器初始为 `suspended`，首次用户交互（点击发送/录音/播放）时调用 `ctx.resume()`；所有 `audio.play()` 必须 `.catch` 处理被自动播放策略拦截的情况（现状 `web.html:33` 已有兜底，延续之）→ 显示可点击重播的播放按钮（手动触发时 resume + play）。
...
| 非 secure context（局域网 http 非 localhost） | `getUserMedia` 直接失败——录音按钮禁用 + tooltip「录音仅支持 localhost 或 HTTPS」（延续现状行为，README/DEPLOYMENT 已知限制） |
...
README「功能特性」「快速开始」「项目结构」三处同步更新（Web 部分加 npm 前置步骤，目录结构换为新工程布局）；测试数量描述同步核对修正。
